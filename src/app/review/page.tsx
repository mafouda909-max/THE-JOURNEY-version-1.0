...
export const metadata: Metadata = {
  ...
  description:
    "طابور مراجعة العروض وطلبات التواصل — فريق الثقة في منصة الرحلة.",
  robots: { index: false },
};
...
export default async function ReviewPage() {
  ...
  const [{ pending, rejected }, contacts, stats, funnel, verificationQueue] =
    await Promise.all([
      getReviewQueue(),
      ...
      db
        ...
        .limit(20)
        .then((rows) =>
          rows.map((r) => ({ ...r.agent, accountEmail: r.accountEmail })),
        ),
    ]);
    ...
    <div className="mx-auto max-w-7xl px-5 pb-24 pt-12 md:px-8 md:pt-16">
      <header className="mb-12">
        ...
        <p className="mt-4 max-w-2xl leading-relaxed text-slate">
          كل عرض جديد يقف في هذا الطابور قبل أن يراه مسافر واحد. اعتمد ما يستوفي
          السياسة، وارفض بمبرر واضح يصل للوكيل.
        </p>
        <div className="mt-6 inline-flex items-start gap-2 rounded-lg border border-outlinev bg-low px-4 py-3 text-[12px] leading-relaxed text-slate">
          ...
          بوابة إدارية محمية. راجع المستندات وسجل القرارات قبل اعتماد التوثيق.
        </div>
      </header>
      ...
      <section className="mt-20 border-t border-outlinev pt-14">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate">
          الحسابات المسجلة ذاتياً تبقى خارج الدليل العام حتى قرارك الموثَّق. كل
          اعتماد ورفض وإيقاف يسجل في سجل القرارات مع السبب.
        </p>
