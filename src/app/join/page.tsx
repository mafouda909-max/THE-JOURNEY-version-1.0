...
const MODES: { key: Mode; title: string; hint: string }[] = [
  ...
  {
    key: "signup-agent",
    title: "حساب وكيل جديد",
    hint: "سير عمل التوثيق يبدأ بعد التفعيل",
  },
  {
    key: "signup-traveler",
    title: "حساب مسافر",
    hint: "تابع طلباتك ومحفوظاتك",
  },
];
...
function JoinForm() {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (mode !== "login") {
      action = "signup";
      payload.passwordConfirmation = String(
        ...
        {MODES.map((m) => (
          <button
            ...
            className={`px-2 py-2.5 text-[12px] font-bold transition-colors ${
              mode === m.key
                ? "bg-deep text-white"
                : "bg-cloud text-slate hover:text-deep"
            }`}
          >
            {m.key === "login"
              ? "دخول"
        ...
        {mode !== "login" && (
          <>
            ...
            {mode === "signup-agent" && (
              <input
                name="city"
                placeholder="المدينة (الرياض، جدة…)"
                className={field}
              />
            )}
          </>
