...
const STATUS_UI: Record<string, { label: string; cls: string; note: string }> =
  {
    pending: {
      ...
      note: "أكمل بياناتك وتحقق من بريدك وأرفق الوثائق ثم أرسل ملفك للمراجعة.",
    },
    in_review: {
      label: "قيد المراجعة",
...
export default async function AccountPage() {
  ...
  const statusUi = agent
    ? (STATUS_UI[agent.verificationStatus] ?? STATUS_UI.pending)
    : null;
  ...
  return (
    <div className="mx-auto max-w-5xl px-5 pb-24 pt-10 md:px-8">
      <div className="mb-10 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-inkwell md:text-4xl">
            مرحباً، {account.displayName}
          </h1>
      ...
      {agent && statusUi && (
        <div
          className={`mb-10 flex items-start gap-4 rounded-2xl border border-outlinev p-6 ${statusUi.cls} bg-opacity-100`}
        >
          ...
          <div>
            <div className="text-lg font-bold">
              حالة التوثيق: {statusUi.label}
            </div>
            <p className="mt-1.5 max-w-2xl text-[14px] leading-relaxed opacity-80">
              {statusUi.note}
            </p>
          </div>
        </div>
            ...
            {myNotifications.slice(0, 6).map((n) => (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[14px] font-bold text-inkwell">
                    {n.title}
                  </span>
      ...
      {account.role === "agent" && agent && (
        <>
          ...
          <section className="mb-12">
            ...
            {agent.verificationStatus !== "verified" ? (
              <div className="rounded-xl border border-dashed border-outlinev bg-cloud px-6 py-10 text-center text-[14px] text-slate">
                نشر العروض يتاح بعد اعتماد التوثيق — هذه القاعدة تحمي المسافر
                قبل الوكيل.
              </div>
            ) : myOffers.length === 0 ? (
              <div className="rounded-xl border border-dashed border-outlinev bg-cloud px-6 py-10 text-center">
                ...
                <p className="mt-2 text-sm text-slate">
                  أنشئ عرضك من النموذج أعلاه ثم انتظر قرار المراجعة.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {myOffers.map((o) => (
                  <div
                    key={o.id}
                    className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-outlinev bg-cloud p-4"
                  >
                    <div>
                      <div className="font-bold text-inkwell">{o.title}</div>
                      <div className="mt-1 text-[12px] text-slate">
                        {tripTypeLabel(o.tripType)} ·{" "}
                        <span className="tnum">
                          {formatMoney(o.priceAmount, o.currency)}
                        </span>{" "}
                        {PRICE_TYPE_LABELS[o.priceType]}
                        {o.status === "rejected" && o.rejectionReason && (
                          <span className="ms-2 text-error">
                            مرفوض: {o.rejectionReason.slice(0, 80)}…
                          </span>
                        )}
                      </div>
                    </div>
          ...
          <section>
            <h2 className="mb-5 text-2xl font-bold text-inkwell">
              طلبات التواصل الواردة ({myLeads.length})
            </h2>
            {myLeads.length === 0 ? (
              ...
              <div className="space-y-3">
                {myLeads.map((l) => (
                  <div
                    key={l.id}
                    className="rounded-xl border border-outlinev bg-cloud p-4"
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="font-bold text-inkwell">
                        {l.travelerName}
                      </div>
                      <span
                        className={`rounded-md px-2.5 py-1 text-[11px] font-bold ${l.status === "new" ? "bg-amber text-gold" : "bg-low text-slate"}`}
                      >
                        {leadStatus[l.status] ?? l.status}
                      </span>
                    </div>
                    <div className="mt-1 font-mono text-[11px] text-slate">
                      {timeAgo(l.createdAt)} · {l.travelerCount} مسافرين ·{" "}
                      {l.travelDates ?? "تواريخ مفتوحة"}
                    </div>
                    <p className="mt-2 line-clamp-2 text-[13px] leading-relaxed text-slate">
                      {l.message}
                    </p>
                    {l.response && (
                      <p className="mt-3 rounded bg-wash p-3">
                        رد الوكيل: {l.response}
                      </p>
                    )}
                    <ContactActions
                      id={l.id}
        ...
        <section>
          ...
          {myLeads.length === 0 ? (
            <div className="rounded-xl border border-dashed border-outlinev bg-cloud px-6 py-10 text-center">
              ...
              <Link
                href="/offers"
                className="mt-4 inline-flex items-center gap-2 rounded-lg bg-deep px-5 py-2.5 text-sm font-bold text-white hover:bg-horizon"
              >
            ...
            <div className="space-y-3">
              {myLeads.map((l) => (
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <Link
                      href={`/offers/${l.offerId}`}
                      className="font-bold text-deep hover:underline"
                    >
                      طلب #{l.id} — تفاصيل العرض
                    </Link>
                    <span
                      className={`inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[11px] font-bold ${l.status === "new" ? "bg-amber text-gold" : l.status === "responded" ? "bg-verifiedbg text-verified" : "bg-low text-slate"}`}
                    >
                  ...
                  <div className="mt-1 font-mono text-[11px] text-slate">
                    {timeAgo(l.createdAt)} · {l.travelDates ?? "تواريخ مفتوحة"}
                  </div>
                  <p className="mt-2 line-clamp-2 text-[13px] leading-relaxed text-slate">
                    {l.message}
                  </p>
