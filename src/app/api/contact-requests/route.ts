...
export async function POST(request: Request) {
  ...
  const offerRows = await db
    ...
    .where(and(eq(offers.id, parsedOfferId), publicOfferFilter()))
    .limit(1);
  const offer = offerRows[0]?.offer;
  if (!offer || offer.status !== "published") {
    return NextResponse.json(
      { error: "هذا العرض لم يعد متاحاً." },
      { status: 404 },
    );
  }
  ...
  if (typeof message !== "string" || message.trim().length < 10) {
    return NextResponse.json(
      {
        error:
          "اكتب رسالة من عشرة أحرف على الأقل — سؤال حقيقي يستحق رداً حقيقياً.",
      },
      { status: 422 },
    );
  }
  ...
  if (
    ...
    count < offer.minTravelers ||
    count > offer.maxTravelers
  ) {
    return NextResponse.json(
      {
  ...
  const [created] = await db
    ...
    .values({
      ...
      travelerAccountId: account?.id ?? null,
