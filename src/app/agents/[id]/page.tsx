    ...
    <div className="mx-auto max-w-7xl px-5 pb-24 pt-10 md:px-8">
        ...
        <div className="rounded-2xl border border-outlinev bg-cloud p-6 md:p-10">
          <div className="flex flex-col gap-8 md:flex-row">
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-3">
                <h1 className="text-3xl font-bold tracking-tight text-inkwell md:text-4xl">
                  {agent.displayName}
                </h1>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-[13px] text-slate">
                ...
                <span className="inline-flex items-center gap-1">
                  <MapPin className="h-3.5 w-3.5 text-deep" /> {agent.city}، {agent.country}
                </span>
                {agent.licenseNumber && (
                  <span className="tnum inline-flex items-center gap-1 font-mono text-[12px]">
                    <BadgeCheck className="h-3.5 w-3.5 text-verified" />
                    رخصة {agent.licenseNumber}
                  </span>
                )}
              </div>
              <p className="mt-5 max-w-3xl leading-[1.9] text-inkwell/80">{agent.bio}</p>
              <div className="mt-5 flex flex-wrap items-center gap-2">
                {agent.specialtyTags.map((t) => (
                  <span key={t} className="rounded-md bg-wash px-3 py-1.5 text-[12px] font-semibold text-deep">
                    {t}
                  </span>
                ))}
          ...
          <div className="mt-8 grid grid-cols-2 gap-3 border-t border-low pt-8 sm:grid-cols-3 lg:grid-cols-6">
            {[
              ...
              <div key={s.l} className="rounded-xl bg-low px-3 py-4 text-center">
                <div className="tnum truncate text-lg font-bold text-deep">{s.v}</div>
                <div className="mt-1 text-[11px] text-slate">{s.l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mt-20">
        <h2 className="mb-8 text-2xl font-bold text-inkwell md:text-3xl">
          العروض المنشورة ({offerCards.length})
        </h2>
        {offerCards.length === 0 ? (
          <div className="rounded-xl border border-dashed border-outlinev bg-cloud px-8 py-14 text-center text-slate">
            لا عروض منشورة لهذا الوكيل حالياً — عروضه القادمة قيد المراجعة.
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {offerCards.map((o) => (
              <OfferCard key={o.id} offer={o} rating={agent.avgRating} />
            ))}
          </div>
        )}
      </section>
      ...
      <section className="mt-20">
        <h2 className="mb-8 flex items-center gap-3 text-2xl font-bold text-inkwell md:text-3xl">
          التقييمات
          <span className="tnum text-lg font-semibold text-slate">
            ★ {agent.avgRating} · {agent.reviewCount}
          </span>
        </h2>
        {agent.reviews.length === 0 ? (
          <div className="rounded-xl border border-dashed border-outlinev bg-cloud px-8 py-14 text-center text-slate">
            لا تقييمات بعد — تُفتح نافذة التقييم بعد ٢٤ ساعة من أول طلب تواصل.
          </div>
        ) : (
            ...
            {agent.reviews.map((r, i) => (
              <Reveal key={r.id} delay={Math.min(i * 0.05, 0.2)}>
                <div className="flex h-full flex-col rounded-xl border border-outlinev bg-cloud p-6">
                  ...
                  <p className="mt-4 flex-1 leading-[1.85] text-inkwell/85">“{r.content}”</p>
                  <div className="mt-5 flex items-center justify-between border-t border-low pt-4 text-[12px] text-slate">
                    <span className="font-semibold text-inkwell">{r.reviewerName}</span>
