...
export function RouteMark({ className = "h-7 w-7" }: { className?: string }) {
  return (
    <svg viewBox="0 0 32 32" className={className} aria-hidden>
      <path d="M4 27 C 13 27, 10 13, 19 13 C 24 13, 26 10, 26.4 8.5" fill="none" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" />
  ...
  return (
    <span className="flex items-center gap-3">
      ...
      <span className="leading-none">
        <span className={`block text-[22px] font-bold tracking-tight ${light ? "text-white" : "text-deep"}`}>الرحلة</span>
        <span className={`mt-1 block font-mono text-[9px] font-semibold uppercase tracking-[0.32em] ${light ? "text-oninverse/50" : "text-slate"}`}>The Journey</span>
      </span>
    </span>
  );
}
    ...
    <>
      <header className="sticky top-0 z-[80] border-b border-outlinev bg-cloud/85 backdrop-blur-md">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 md:h-[72px] md:px-8">
          <Link href="/" aria-label="الرحلة — الرئيسية"><Wordmark /></Link>
          <nav className="hidden items-center gap-8 md:flex" aria-label="التنقل الرئيسي">
            {links.map((l) => (
              <Link key={l.href} href={l.href} className={`relative text-[15px] font-medium transition-colors hover:text-deep ${pathname === l.href ? "text-deep" : "text-slate"}`}>
                {l.label}
                {pathname === l.href && <span className="absolute -bottom-2 right-0 h-0.5 w-full rounded-full bg-deep" />}
              </Link>
            ))}
          </nav>
          <div className="flex items-center gap-3">
            <Link href="/join" className="hidden text-sm font-semibold text-slate transition-colors hover:text-deep md:block">دخول</Link>
            <Link href="/join?mode=agent" className="hidden rounded-lg border-2 border-deep px-5 py-2.5 text-sm font-semibold text-deep transition-all duration-300 hover:bg-deep hover:text-white md:block">انضم كوكيل</Link>
            <button
              type="button"
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            <div className="px-8 pb-10 text-sm text-slate">منصّة الوكلاء الموثّقين — معلومات الثقة بجانب كل عرض.</div>
          </motion.div>
        )}
      </AnimatePresence>
    </>
      ...
      <div className="mx-auto max-w-7xl px-5 py-16 md:px-8">
        <div className="grid grid-cols-1 gap-12 md:grid-cols-12">
          <div className="md:col-span-5">
            <Wordmark light />
            <p className="mt-6 max-w-sm leading-relaxed text-oninverse/60">سوق ثقة للسفر: المسافر يتواصل مباشرة مع الوكيل، والرحلة تتوسّط التوثيق والمراجعة والمتابعة — لا الحجز.</p>
            <a href="mailto:hello@alrihla.travel" className="mt-6 inline-flex items-center gap-2 rounded-lg border border-oninverse/25 px-4 py-2.5 text-sm transition-colors hover:border-white hover:text-white">
              <Mail className="h-4 w-4" />
              hello@alrihla.travel
            </a>
          </div>
          <div className="md:col-span-2">
            <h4 className="mb-5 font-mono text-[11px] uppercase tracking-[0.2em] text-oninverse/40">المنصّة</h4>
          ...
          <div className="md:col-span-2">
            <h4 className="mb-5 font-mono text-[11px] uppercase tracking-[0.2em] text-oninverse/40">الثقة والقانون</h4>
            <ul className="space-y-3 text-sm text-oninverse/75">
          ...
          <div className="md:col-span-3">
            <h4 className="mb-5 font-mono text-[11px] uppercase tracking-[0.2em] text-oninverse/40">ما نراجعه</h4>
            <ul className="space-y-3 text-sm text-oninverse/75">
              <li className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-verified" />أدلة الهوية المطلوبة للوكيل</li>
              <li className="flex items-center gap-2"><BadgeCheck className="h-4 w-4 text-verified" />مستندات النشاط حسب نوع الحساب</li>
              <li className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-verified" />مراجعة كل عرض قبل النشر</li>
            </ul>
          </div>
        </div>
        <div className="mt-14 flex flex-col items-start justify-between gap-3 border-t border-white/10 pt-6 font-mono text-[10px] uppercase tracking-[0.2em] text-oninverse/40 md:flex-row md:items-center">
          ...
          <span>قارن العرض · افهم الثقة · تواصل مباشرة</span>
        </div>
      </div>
    </footer>
  );
