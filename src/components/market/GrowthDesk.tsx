      ...
      <div className="mt-4 flex flex-wrap gap-3">
        {data.leadsBySource.map((s) => (
          <div key={s.source} className="rounded-xl border border-outlinev bg-cloud px-5 py-4">
            ...
            <div className="mt-1 text-[12px] font-semibold text-slate">{s.source}<span className="tnum text-slate/50"> · {totalLeads > 0 ? Math.round((s.count / totalLeads) * 100) : 0}%</span></div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        {data.campaigns.map((c) => (
          <div key={c.id} className="rounded-xl border border-outlinev bg-cloud p-5">
            <div className="flex items-center justify-between"><div className="font-bold text-inkwell">{c.name}</div><StatusChip status={c.status} /></div>
            <p className="mt-2 text-[13px] leading-relaxed text-slate">{c.objective}</p>
            <div className="mt-3 flex flex-wrap gap-1.5">{c.channels.map((ch) => <span key={ch} className="rounded-md bg-low px-2 py-1 text-[11px] font-semibold text-slate">{CHANNEL_LABELS[ch] ?? ch}</span>)}</div>
            <p className="mt-3 border-t border-low pt-3 text-[12px] leading-relaxed text-slate"><b className="text-inkwell">الفرضية:</b> {c.hypothesis}<br /><b className="text-inkwell">KPI:</b> {c.kpi}</p>
          </div>
        ))}
      </div>
        ...
        {data.content.map((item) => {
          ...
          return (
            <div key={item.id} className="rounded-xl border border-outlinev bg-cloud p-5">
              <div className="flex flex-wrap items-center gap-2"><StatusChip status={item.status} /><span className="rounded-md bg-low px-2 py-1 text-[11px] font-semibold text-slate">{CHANNEL_LABELS[item.channel] ?? item.channel}</span>{item.risk !== "low" && <span className={`inline-flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-semibold ${RISK_STYLES[item.risk]}`}><ShieldAlert className="h-3 w-3" />خطورة {RISK_LABELS[item.risk]}</span>}<span className="font-mono text-[11px] text-slate/60">{timeAgo(item.createdAt)}</span></div>
              ...
              <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-low pt-3">
                <div className="text-[12px] text-slate">{item.cta ? <>CTA: <span className="font-semibold text-deep">{item.cta}</span></> : item.performanceNote ? <span className="inline-flex items-center gap-1.5 text-stone"><TrendingUp className="h-3.5 w-3.5" />{item.performanceNote}</span> : "—"}</div>
                {next && <button onClick={() => void act({ entity: "content", id: item.id, to: next.to }, key)} disabled={busy !== null} className="inline-flex items-center gap-2 rounded-lg bg-deep px-4 py-2 text-[12px] font-bold text-white transition-colors hover:bg-horizon disabled:opacity-50">{busy === key ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <CheckCircle2 className="h-3.5 w-3.5" />}{next.label}</button>}
              </div>
            </div>
          );
        })}
